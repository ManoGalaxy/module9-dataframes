...
# --- We cant use $ sign, it thinks its talking about an index, it can only read strings a column name, ---
# ---otherwise it will think its a number" ---
...
care.spent.1940 <- us.exp['Personal Care', 'X1940']

# How much money was spent on Food and Tobacco in 1960?
spent.fdtb.1960 <- us.exp['Food and Tobacco', 'X1960']

# What was the highest expenditure category in 1960?
# Hint: use the `max()` function to find the largest, then compare that to the column
highest.exp <- us.exp$category[us.exp$X1960 == max(us.exp$X1960)]


# Define a function `DetectHighest` that takes in a year as a parameter, and
# returns the highest spending category of that year
DetectHighest <- function(highest.year) {
